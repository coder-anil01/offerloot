            ...
            <Slider {...settings}>
                {category?.map((c)=>(
                    <div key={c._id} className='h-c-card'>
                        <Link><img className='h-c-image' src={c.image} alt={c.name} /></Link>
...
{/* PRODUCT SHOW */}
        ...
        {product?.map((p)=> (
            <div key={p._id} className='product-box-card'>
                <div className='product-box-image'><img src={p.image} alt={p.title.slice(0, 10)} /></div>
                <div className='product-box-title'>{p.title}</div>
